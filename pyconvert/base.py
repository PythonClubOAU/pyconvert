...
from pyconvert.errors import UnitError

__all__ = {
    "measurement": {
        "length": {"standard_unit": "metres", "standard_symbol": "m", "available_converts": ["metres",
        ...
        "mass": {"standard_unit": "gramme", "standard_symbol": "g", "available_converts": ["grammes",
                                                                                           "kilogrammes",
                                                                                           "pounds"],
                 "available_units": ["g", "kg", "p"],
                 "converts_and_units": {"grammes": "g", "kilogrammes": "kg", "pounds": "p"}},

        "volume": {"standard_unit": "volume", "standard_symbol": "V", "available_converts": ["metre_cube",
                                                                                             "centimetre_cube",
                                                                                             "millimeter_cube",
                                                                                             "kilometer_cube"],
        ...
        "speed": {"standard_unit": "metre_per_second", "standard_symbol": "m/s", "available_converts": [
            "metres_per_second",
        ...
        "currency": {"standard_unit": "Naira", "standard_symbol": "N", "available_converts": ["naira", "rand", "rand",
                                                                                              "dollars"],
                     "available_units": ["N", "$", "Y", "R"],
                     "converts_and_units": {"naira": "N", "dollars": "$", "Yuan": "Y", "rand": "R"}},

        "temperature": {"standard_unit": "Kelvin", "standard_symbol": "K", "available_converts": ["Kelvin",
                                                                                                  "Celsius",
                                                                                                  "Farheint"],
...
class Validator():
    ...
    def is_type_valid(name):
        if not isinstance(name, str):
            raise TypeError("The unit {0} is not of type str. All unit entered must be strings".format(name))
        else:
            return True

    def is_name_valid(unit, value):
        if value in __all__[unit]:
            return True
        else:
            raise UnitError("The unit {0} does not exist.".format(value))
